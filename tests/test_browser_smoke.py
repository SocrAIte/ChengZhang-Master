...
class BrowserSmokeTest(unittest.TestCase):
    ...
    def test_console_browser_smoke_happy_path_with_fake_browser(self) -> None:
        body = "Market Impact Radar Console Daily Runs Dashboard Data 2026-05-15 Run Date Refresh Runs Signal Search Risk Filter Status Filter Sort Signals Grouped by theme Flat signal list Theme Hotlist Signal Detail Panel Evidence Chain Data Quality / Freshness ETF observation pool Stock observation pool Artifact Links Historical Review Historical Theme Trends Recurring Observation Candidates Data Quality Trend API: API version: Dashboard Knowledge Review Run Diagnostics"
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
