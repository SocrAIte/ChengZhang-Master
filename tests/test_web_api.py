...
class WebApiTest(unittest.TestCase):
    ...
    def test_console_endpoint_returns_static_readonly_shell_without_file_access(self) -> None:
        ...
        self.assertIn("跨市场热点研究控制台", response.payload)
        self.assertIn("只读 daily report 研究工作台", response.payload)
        self.assertIn("工作台导航", response.payload)
        self.assertIn("ui-language-toggle", response.payload)
        self.assertIn("uiLang", response.payload)
        self.assertNotIn("在只读分析区域之间快速跳转", response.payload)
        for anchor in (
            "#console-usage-guide",
            "#morning-brief",
        ...
        self.assertIn("产物链接", response.payload)
        self.assertIn("知识图谱复核", response.payload)
        self.assertIn("运行诊断", response.payload)
        self.assertIn("运行日期", response.payload)
        self.assertIn("刷新运行记录", response.payload)
        self.assertIn("搜索信号", response.payload)
        self.assertIn("风险筛选", response.payload)
        self.assertIn("状态筛选", response.payload)
        self.assertIn("信号排序", response.payload)
        self.assertIn("按主题分组", response.payload)
        self.assertIn("平铺信号列表", response.payload)
        self.assertIn("主题热榜", response.payload)
        self.assertIn("今日观察摘要", response.payload)
        self.assertIn("使用引导", response.payload)
        self.assertIn("先看今日摘要", response.payload)
        self.assertIn("查看研究复核清单", response.payload)
        self.assertIn("查看主题 × 来源矩阵", response.payload)
        self.assertIn("使用日期对比", response.payload)
        self.assertIn("生成研究笔记", response.payload)
        self.assertIn("每日研究摘要", response.payload)
        self.assertIn("研究复核清单", response.payload)
        self.assertIn("默认显示前 10 条复核事项", response.payload)
        self.assertIn("研究笔记工作区", response.payload)
        self.assertIn("research-notes-composer", response.payload)
        self.assertIn("笔记语言", response.payload)
        self.assertIn("笔记模式", response.payload)
        self.assertIn("完整笔记", response.payload)
        self.assertIn("简洁笔记", response.payload)
        self.assertIn("人工补充笔记", response.payload)
        self.assertIn("manual-research-notes", response.payload)
        self.assertIn("人工补充笔记仅保留在当前浏览器页面中", response.payload)
        ...
        self.assertIn("研究包导出", response.payload)
        self.assertIn("research-export-package", response.payload)
        self.assertIn("导出内容", response.payload)
        self.assertIn("导出预览", response.payload)
        self.assertIn("下载 Markdown", response.payload)
        self.assertIn("下载纯文本", response.payload)
        self.assertIn("下载 JSON 元信息", response.payload)
        ...
        self.assertIn("复核清单摘要", response.payload)
        self.assertIn("严重程度", response.payload)
        self.assertIn("复核类型", response.payload)
        self.assertIn("范围", response.payload)
        ...
        self.assertIn("日期对比", response.payload)
        ...
        self.assertIn("对比起始日期", response.payload)
        self.assertIn("对比目标日期", response.payload)
        self.assertIn("日期对比观察", response.payload)
        self.assertIn("daily-research-brief", response.payload)
        self.assertIn("复制 Markdown", response.payload)
        self.assertIn("复制纯文本", response.payload)
        self.assertIn("摘要语言", response.payload)
        self.assertIn("中文", response.payload)
        self.assertIn("摘要模式", response.payload)
        self.assertIn("完整摘要", response.payload)
        self.assertIn("简洁摘要", response.payload)
        self.assertIn("包含小节", response.payload)
        ...
        self.assertIn("浏览器本地生成", response.payload)
        self.assertIn("不保存到服务器", response.payload)
        self.assertIn("研究观察", response.payload)
        self.assertIn("数据来源可靠性", response.payload)
        self.assertIn("来源详情", response.payload)
        self.assertIn("来源汇总", response.payload)
        self.assertIn("弱证据信号", response.payload)
        self.assertIn("历史数据质量趋势", response.payload)
        self.assertIn("主题 × 来源矩阵", response.payload)
        self.assertIn("矩阵摘要", response.payload)
        self.assertIn("单元格详情", response.payload)
        self.assertIn("需要复核的主题-来源组合", response.payload)
        self.assertIn("主题对比", response.payload)
        self.assertIn("候选池对比", response.payload)
        self.assertIn("返回顶部", response.payload)
        ...
        self.assertIn("主题详情", response.payload)
        self.assertIn("theme-button", response.payload)
        self.assertIn("信号详情", response.payload)
        self.assertIn("证据链", response.payload)
        self.assertIn("数据质量", response.payload)
        self.assertIn("ETF 观察池", response.payload)
        self.assertIn("个股观察池", response.payload)
        self.assertIn("历史复盘", response.payload)
        self.assertIn("历史主题趋势", response.payload)
        self.assertIn("反复出现的观察候选", response.payload)
        self.assertIn("数据质量趋势", response.payload)
        self.assertIn("API 状态和版本", response.payload)
        self.assertIn("产物链接", response.payload)
        self.assertIn("当前筛选条件下没有匹配信号。", response.payload)
